...
const Reports = () => {
  ...
  const schools = [
    'All Hostels',
    'Government Hostel Bangalore North',
    'Government Hostel Mysore',
    'Government Hostel Hubli',
    'Government Hostel Mangalore'
  ];
  ...
  const foodInspectionReports = [
    {
      id: 1,
      location: "Government Hostel, Bangalore North",
      date: "2024-01-15",
    ...
    {
      id: 2,
      location: "Government Hostel, Mysore",
      date: "2024-01-18",
    ...
    {
      id: 3,
      location: "Government Hostel, Hubli",
      date: "2024-01-20",
      ...
      <div className="space-y-4 sm:space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 sm:mb-6 gap-3 sm:gap-4">
          <div className="min-w-0 flex-1">
            <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-800">Inspection Photo Gallery</h2>
            <p className="text-gray-600 text-sm sm:text-base">View recent inspection photos from hostels</p>
          </div>
          <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 w-full md:w-auto">
            <div className="w-full sm:w-auto md:w-64 flex-1 sm:flex-initial">
              <label className="block text-sm font-medium text-gray-700 mb-1">Filter by Hostel</label>
              <select
                value={selectedSchool}
                onChange={(e) => setSelectedSchool(e.target.value)}
                className="w-full p-2.5 sm:p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-base"
              >
                {schools.map((school) => (
        ...
        {filteredInspections.length === 0 ? (
          <div className="text-center py-8 sm:py-12 bg-white rounded-lg shadow p-4">
            <p className="text-gray-500 text-sm sm:text-base">No inspection photos found matching the selected filters.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {filteredInspections.map((inspection) => (
              <div key={inspection.id} className="bg-white rounded-lg shadow-md overflow-hidden active:shadow-lg sm:hover:shadow-lg transition-shadow touch-manipulation">
                <div className="relative h-40 sm:h-48 bg-gray-100">
                  ...
                  <div className="absolute top-2 left-2 bg-white bg-opacity-90 px-2 py-1 rounded-md shadow-sm">
                    <span className={`font-semibold text-xs sm:text-sm ${inspection.rating === 'A+' ? 'text-green-600' :
                ...
                <div className="p-3 sm:p-4">
                  <h3 className="font-semibold text-gray-800 mb-1 truncate text-sm sm:text-base">{inspection.location}</h3>
  ...
  const PhotoModal = ({ photo, onClose, onNext, onPrev, hasNext, hasPrev }) => {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-90 z-50 flex items-center justify-center p-2 sm:p-4">
        <button
          onClick={onClose}
          className="absolute top-2 right-2 sm:top-4 sm:right-4 text-white active:text-gray-300 sm:hover:text-gray-300 min-w-[44px] min-h-[44px] flex items-center justify-center touch-manipulation"
          aria-label="Close"
        >
        ...
        {hasPrev && (
          <button
            onClick={onPrev}
            className="absolute left-2 sm:left-4 bg-black bg-opacity-50 text-white p-2 sm:p-3 rounded-full active:bg-opacity-75 sm:hover:bg-opacity-75 transition-all touch-manipulation min-w-[44px] min-h-[44px] flex items-center justify-center"
        ...
        <div className="relative max-w-4xl w-full">
          <img
            ...
            className="max-h-[calc(100vh-8rem)] sm:max-h-[80vh] w-auto mx-auto object-contain"
          />
          <div className="mt-3 sm:mt-4 text-white text-center px-2">
            <h3 className="text-base sm:text-lg lg:text-xl font-semibold mb-1 break-words">{photo.school}</h3>
            <p className="text-sm sm:text-base text-gray-300 break-words">{photo.caption}</p>
            <p className="text-xs sm:text-sm text-gray-400 mt-2">{new Date(photo.date).toLocaleDateString()}</p>
          </div>
        </div>
        ...
        {hasNext && (
          <button
            onClick={onNext}
            className="absolute right-2 sm:right-4 bg-black bg-opacity-50 text-white p-2 sm:p-3 rounded-full active:bg-opacity-75 sm:hover:bg-opacity-75 transition-all touch-manipulation min-w-[44px] min-h-[44px] flex items-center justify-center"
  ...
  return (
    <div className="p-2 sm:p-4 lg:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-1 sm:mb-2">Reports & Analytics</h1>
        <p className="text-gray-600 text-sm sm:text-base">Generate comprehensive reports and analyze scheme performance</p>
      </div>
          ...
          {reportTypes.map((type) => {
            ...
            return (
              <button
                ...
                className={`w-full sm:w-auto flex items-center justify-between sm:justify-start gap-2 px-4 py-3 rounded-xl border min-h-[48px] sm:min-h-[44px] touch-manipulation text-sm sm:text-base transition-all ${isActive
                  ? 'bg-blue-50 border-blue-200 text-blue-700 font-semibold shadow-sm'
                  : 'bg-gray-50 border-gray-100 text-gray-600 active:bg-gray-100 sm:hover:bg-gray-100'
                  }`}
              >
                <type.icon className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" />
                <span className="flex-1 text-left sm:text-center whitespace-nowrap">{type.name}</span>
              </button>
            );
          })}
        </div>
      ...
      <div className="space-y-4 sm:space-y-6">
        {reportType === 'overview' && (
          <div>
            <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
              <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-3 sm:mb-4">Category Distribution</h3>
            ...
            <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mt-4 sm:mt-6">
              <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-3 sm:mb-4">Recent Inspections</h3>
              <div className="space-y-3 sm:space-y-4">
                {foodInspectionReports.map((report) => (
                  <div key={report.id} className="border-b pb-3 sm:pb-4 last:border-0 last:pb-0">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0">
                      <h4 className="font-medium text-gray-900 text-sm sm:text-base break-words flex-1">{report.location}</h4>
                      <span className={`px-2 py-1 text-xs rounded-full whitespace-nowrap ${report.rating === 'A+' ? 'bg-green-100 text-green-800' :
                    ...
                    {report.photos.length > 0 && (
                      <div className="mt-2 flex space-x-2 overflow-x-auto pb-2 -mx-1 px-1">
                        {report.photos.slice(0, 3).map((photo, idx) => (
                          <div key={idx} className="flex-shrink-0 w-16 h-16 sm:w-20 sm:h-20 bg-gray-100 rounded overflow-hidden touch-manipulation">
                            <img
                              src={photo.url}
                              alt={photo.caption}
                              className="w-full h-full object-cover cursor-pointer"
                        ...
                        {report.photos.length > 3 && (
                          <div className="flex-shrink-0 w-16 h-16 sm:w-20 sm:h-20 bg-gray-100 rounded flex items-center justify-center text-gray-500 text-xs sm:text-sm">
        ...
        {reportType === 'financial' && (
          <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
            <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-3 sm:mb-4">Financial Reports</h3>
            <p className="text-gray-600 text-sm sm:text-base">Financial reports will be displayed here.</p>
          </div>
        )}

        {reportType === 'beneficiary' && (
          <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
            <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-3 sm:mb-4">Beneficiary Reports</h3>
            <p className="text-gray-600 text-sm sm:text-base">Beneficiary reports will be displayed here.</p>
          </div>
        )}

        {reportType === 'audit' && (
          <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
            <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-3 sm:mb-4">Audit Reports</h3>
            <p className="text-gray-600 text-sm sm:text-base">Audit reports will be displayed here.</p>
          </div>
        )}
      </div>
